...
class model:
    ...
    def setup(self):
      #  base = tf.keras.applications.MobileNetV3Small(input_shape=(200,200,3),include_top=False,weights='imagenet')
        ...
        extra_inputs = tf.keras.Input(shape=(3,))
        ...
        concatenated  = tf.keras.layers.Concatenate()([flatten_layer,extra_inputs])
        fc_layer_1 = tf.keras.layers.Dense(units=128,activation='relu')(concatenated)
        fc_layer_2 = tf.keras.layers.Dense(units=64,activation='relu')(fc_layer_1)
        output = tf.keras.layers.Dense(units=3,activation='linear')(fc_layer_2)
        self.model = tf.keras.models.Model(inputs=[input,extra_inputs],outputs=output)
